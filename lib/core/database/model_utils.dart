  ...
  static toChTableSql(dynamic v) {
    ...
    variables.forEach((dm) {
      ...
      final columnDef = dm.metadata.firstWhere(
          ...
          orElse: () => null);
      Column cDef = columnDef?.reflectee as Column;
      Object defaultValue = cDef?.defaultValue ?? null;
      bool nullable = (cDef)?.nullable ?? true;
      bool isId = (cDef)?.id ?? false;
...
//      }
    ...
    cols.sort();
    // 将id放在最前面
    int idIndex = cols.indexWhere((x) => x.contains('`id`'));
    if (idIndex >= 0) {
      String idCol = cols[idIndex];
      cols.removeAt(idIndex);
      cols.insert(0, idCol);
    }
    String pb = tablePartitionBy(thisMirror);
  ...
  static List<R> resultsMapper<R>(List<dynamic> results) {
    ...
    // 准备declare map
    Map<String, DeclarationMirror> declareMap = {};
    variables.forEach((syb) {
      String varName = MirrorSystem.getName(syb.simpleName);
      // 映射原始名称
      declareMap[varName] = syb;
      // 映射_名称
      String colName = toSqlColumnName(varName);
      if (colName != varName) {
  ...
  static setRFields(
    ...
    VariableMirror dim = variable as VariableMirror;
    String reflectType = dim.type?.reflectedType?.toString() ?? 'String';
    switch (reflectType) {
      case 'bool':
      ...
      case 'String':
        ...
        if (null != enumMetadata && value is int) {
          Map<String, int> _enums = (enumMetadata.reflectee as Enum).enums;
