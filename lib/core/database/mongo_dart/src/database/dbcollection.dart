...
//
  ...
  Future<Map<String, dynamic>> findOne([selector]) {
    ...
    cursor.close();
    print(
        '$collectionName find one finished in ${DateTime.now().millisecondsSinceEpoch - s} mills.');
    return result;
  }
...
//          collectionName, _selectorBuilder2Map(selector), writeConcern);
  ...
  Future<int> count([selector]) {
    ...
    return db
        ...
        .then((reply) {
      print(
          '$collectionName count finished in ${DateTime.now().millisecondsSinceEpoch - s} mills.');
